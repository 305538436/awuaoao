...
const state = reactive({
  ...
  interval: 0,
  heartCount: undefined as CountUp | undefined,
});
// 初始化数字滚动
const initNumCountUp = () => {
  nextTick(() => {
    ...
    topCardItemRefs.value.forEach((v: HTMLDivElement, i: number) => {
      console.log(v.querySelector('.countup-card-item-title-num'))
      if (i === 2) {
        state.heartCount = new CountUp(v.querySelector('.countup-card-item-title-num') as HTMLDivElement, state.topCardItemList[i].titleNum, {
          duration: 3,
        });
        state.heartCount.start()
      } else {
  ...
  state.interval = setInterval(() => {
    state.heartCount?.update(dayjs().diff(dayjs(state.topCardItemList[0].date), 'second') * 1.5);
  }, 2300);
